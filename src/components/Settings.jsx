  ...
  return (
    <Fragment >
        ...
        onSubmit={async (props) => {
          try {
            const filter = filters.filter(
              (filter) => filter.id === props.filter
            )[0];
            console.log("form props:", props);
            console.log("filter jql:", filter.jql);

            const res = await handleTrainJob(filter.jql, "org_02");
            setisTraining(true);
            return res;
          } catch (error) {
      ...
      >
        ...
        {isTraining && (
          <SectionMessage title="Learning job running..." appearance="warning">
            <Text>Esti-mate is learning your estimation patterns</Text>
          </SectionMessage>
        )}

        {!isTraining && (
          <SectionMessage
            title="Learning job completed"
            appearance="confirmation"
          >
            <Text>Esti-mate is ready for estimating your issues</Text>
          </SectionMessage>
        )}
      </Form>
    </Fragment>
...
const handleTrainJob = async (jql, org_id) => {
  try {
    ...
    gs_path =
      gs_path_list[gs_path_list.length - 2] +
      "/" +
      gs_path_list[gs_path_list.length - 1];
